		...
		<div class="row">
				...
				<div class="row">
					<div class="rounded-5 brown-div shadow-lg mt-5 mb-5 p-5">
						...
						<div class="col-12 pt-3 ">
							...
							<p>Rôle : {if ({$objUser->getRole()}=="user")}Utilisateur{elseif ({$objUser->getRole()}=="modo")}Modérateur{elseif ({$objUser->getRole()}=="admin")}Administrateur{/if} </p>
							...
							{if ($user.user_id == $objUser->getId()) || ($smarty.session.user.user_role == "modo") || ($smarty.session.user.user_role == "admin") }
								<div class="row mt-4">
									<div class="col button-center pb-1 ">
										<a href="{$base_url}user/edit_profile" title="Modifier mon compte" class="orange-btn"><i class="pe-2 fa-solid fa-pen-to-square"> </i>Modifier le profil</a>
									</div>
								</div>
							{/if}
						{/if}
			...
			<div class="col-8">
				...
				<div class="row rounded-5 brown-div shadow-lg mt-5 mb-5 p-5 ms-5">
					<div class="mb-5"><h2 class="fs-3 orange">Derniers topics publiés</h2></div>
					<div>
